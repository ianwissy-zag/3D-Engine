...
int getTextureColumnNumberForRay(Vector3f* ray, RayType rtype) {
    int hitX = (fpPlayerPosX >> 16) + (int)ray->x;
    int hitY = (fpPlayerPosY >> 16) + (int)ray->y;

    if(rtype == HORIZONTAL_RAY) {
        if(ray->y < 0)
            return hitX % TEXTURE_SIZE;
        else
            return TEXTURE_SIZE - 1 - (hitX % TEXTURE_SIZE);
    } else {
        if(ray->x > 0)
            return hitY % TEXTURE_SIZE;
        else
            return TEXTURE_SIZE - 1 - (hitY % TEXTURE_SIZE);
    }
}

float getUndistortedRayLength(Vector3f* ray) {
    Vector3f undistortedRay;
    Vector3f dynamicViewplane;
    
    uint8_t planeAngle = playerAngleIndex + 64; 
    dynamicViewplane.x = (float)COS_LUT[planeAngle] / 65536.0f;
    dynamicViewplane.y = (float)SIN_LUT[planeAngle] / 65536.0f;
    dynamicViewplane.z = 1.0f;

    Vector3f proj = vectorProjection(ray, &dynamicViewplane);
